...
url=$(yqgo '.emseq_ref_assemblies.puc19.url' ./config/example-config.yaml) && \
outfile=$(yqgo '.emseq_ref_assemblies.puc19.input' ./config/example-config.yaml) && \
wget -O - --user-agent="Mozilla/5.0" --referer="https://www.neb.com" "$url" | \
  gzip > "$data_dir/inputs/$outfile"


url=$(yqgo '.emseq_ref_assemblies.unmeth_lambda.url' ./config/example-config.yaml) && \
outfile=$(yqgo '.emseq_ref_assemblies.unmeth_lambda.input' ./config/example-config.yaml) && \
wget -O - --user-agent="Mozilla/5.0" --referer="https://www.neb.com" "$url" | \
  gzip > "$data_dir/inputs/$outfile"


url=$(yqgo '.emseq_ref_assemblies.ncbi_decoy_hg38.url' ./config/example-config.yaml) && \
outfile=$(yqgo '.emseq_ref_assemblies.ncbi_decoy_hg38.input' ./config/example-config.yaml) && \
...
*** Methylation sequence processing Version 2
:PROPERTIES:
:header-args:snakemake: :tangle ./workflows/em-seq.v2.smk :tangle-mode (identity #o555)
:END:
**** EM-seq adapter trimming
- Snakemake, inline
  #+begin_src snakemake
...
#+end_src
- Script
  #+begin_src bash :tangle ./scripts/fastp_emseq_wrapper.sh
#!/usr/bin/env bash
set -euo pipefail
...
:PROPERTIES:
:ID:       0063769e-67d9-45c1-a03e-ade92641b127
:header-args:snakemake: :tangle ./workflows/workflow2.smk
:END:
#+begin_src snakemake
<#emseq-dedup#>
<#emseq-pileup#>
...
:end:
#+begin_src snakemake
rule emseq_dupsifter:
    conda:
        "../config/biscuit-conda-env.yaml",
    input:
        bam = f"{data_dir}/analysis/emseq/bams/{{library_id}}.{{ref_name}}.{{align_method}}.namesorted.bam",
        fasta = f"{data_dir}/ref/{{align_method}}/{{ref_name}}/{{ref_name}}.fa",
    log:
        f"{log_dir}/{{library_id}}_{{ref_name}}_{{align_method}}_emseq_dedup.log",
    output:
        bam = f"{data_dir}/analysis/emseq/bams/{{library_id}}.{{ref_name}}.{{align_method}}.dedup.coordsort.bam",
        index = f"{data_dir}/analysis/emseq/bams/{{library_id}}.{{ref_name}}.{{align_method}}.dedup.coordsort.bam.bai",
    shell:
        r"""
        rm -f {output.bam}.tmp.*.bam
        samtools view -h -f 0x2 {input.bam} \
...
rule emseq_align_biscuit:
    ...
    log:
        cmd = f"{data_dir}/logs/{{library_id}}.{{ref_name}}.biscuit.namesorted.bam",
    output:
        bam = f"{data_dir}/analysis/emseq/bams/{{library_id}}.{{ref_name}}.biscuit.namesorted.bam",
    params:
        threads = config["threads"],
    resources:
        concurrency=100
...
rule emseq_biscuit_pileup:
    ...
    input:
        ...
        fasta = f"{data_dir}/ref/{{align_method}}/{{ref_name}}/{{ref_name}}.fa",
    log:
        f"{log_dir}/{{library_id}}_{{ref_name}}_{{align_method}}_emseq_pileup.log",
    output:
        vcf = f"{data_dir}/analysis/emseq/pileup/{{library_id}}.{{ref_name}}.{{align_method}}_pileup.vcf.gz",
        tsv = f"{data_dir}/analysis/emseq/pileup/{{library_id}}.{{ref_name}}.{{align_method}}_pileup.vcf_meth_average.tsv",
    params:
        out_base = f"{data_dir}/analysis/emseq/pileup/{{library_id}}.{{ref_name}}.{{align_method}}_pileup.vcf",
    shell:
        """
        biscuit pileup \
        -d 1 \
