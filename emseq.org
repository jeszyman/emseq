...
- Preprocessing and quality control.
  Raw sequencing reads were processed with fastp (v0.23.4) using default parameters, with adapter detection enabled and quality filtering disabled to preserve methylation signal, as recommended for EM-seq libraries. FastQC (v0.11.9) was used to assess read quality before and after trimming.

- Alignment and deduplication.
  Reads were aligned to the human reference genome (GRCh38) using BWA-meth (v0.2.2) with default parameters. Resulting BAMs were coordinate-sorted and deduplicated using dupsifter (https://github.com/brentp/dupsifter), which performs read name-aware deduplication optimized for bisulfite-style alignments. Only properly paired reads (SAM flag 0x2) were retained prior to deduplication.
