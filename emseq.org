...
rule biscuit_index_genome:
    input:
        fa_gz = f"{data_dir}/inputs/{emseq_fasta}.gz",
    output:
        fa = f"{data_dir}/ref/biscuit-{emseq_assembly}/{emseq_fasta}",
        fai = f"{data_dir}/ref/biscuit-{emseq_assembly}/{emseq_fasta}.fai",
        index = expand(
            f"{data_dir}/ref/biscuit-{emseq_assembly}/{emseq_fasta}{{ext}}",
            ext=BISCUIT_INDEX_EXTS
        )
    shell:
        """
...
rule emseq_biscuit_align:
    input:
        r1 = f"{emseq_dir}/fastqs/{{library}}_trimmed_R1.fastq.gz",
        r2 = f"{emseq_dir}/fastqs/{{library}}_trimmed_R2.fastq.gz",
        fasta = f"{data_dir}/ref/biscuit-{emseq_assembly}/{emseq_ref_fasta}",
    log:
        cmd = f"{log_dir}/{{library}}_emseq_biscuit_align.log",
    output:
        bam = f"{emseq_bam_dir}/{{library}}.bam",
    resources:
        concurrency=100
    shell:
        """
...
#+end_src
**** methylkit
***** Make per-sample coverage files
...
***** Make differential methylation object
#+begin_src snakemake
rule make_methylkit_diff_db:
    input:
        mkit_lib_db = lambda wildcards: expand(
