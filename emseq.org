...
| =haplotype.cpg-ref=  | Tabixed CpG reference file (hg38)                                 |
| =haplotype.mhb-bed=  | Methylation haplotype block BED (e.g. Guo 2017 liftover)          |
...
| =deconv.atlas=       | UXM deconvolution reference atlas TSV                             |
...
- Development since last tag
  - [2026-03-31 Mon] Split README into two-module documentation with separate DAGs and CI badges. Reverted to direct thread/concurrency values per rule (basecamp pattern). Added analysis CI workflows (=smk-analysis-dry=, =smk-analysis-run=). Removed project-specific wrapper files.
...
jobs:
  run:
    ...
    steps:
      ...
      - name: Clone and build external analysis repos
        run: |
          set -euo pipefail
      ...
      - name: Verify required files
        run: |
          ...
          test -f workflows/test-analysis.smk
          test -f config/test.yaml
          test -x "$HOME/repos/mHapTools/mhaptools"
