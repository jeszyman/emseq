...
rule emseq_align_bwameth_spike:
    ...
    threads: 48
    log:
        f"{config['emseq-dir']}/logs/{{library_id}}.{{ref_name}}.emseq_align_bwameth_spike.log"
    params:
...
rule emseq_methyldackel_spike:
    ...
    log:
        f"{config['emseq-dir']}/logs/{{library_id}}_{{ref_name}}_{{align_method}}_methyldackel.log",
    params:
...
rule bwa_meth_index:
    ...
    log:
        f"{config['emseq-dir']}/logs/{{name}}_bwa_meth_index.log"
    shell:
        """
...
rule emseq_align_bwameth:
    ...
    params:
        temp_prefix = lambda wildcards: f"{data_dir}/tmp/{wildcards.library_id}.{wildcards.ref_name}",
    log:
        f"{config['emseq-dir']}/logs/{{library_id}}.{{ref_name}}.bwameth.log"
    shell:
        """
        mkdir -p $(dirname {params.temp_prefix})
        bwameth.py --threads {threads} \
...
rule emseq_methyldackel:
    ...
    log:
        f"{config['emseq-dir']}/logs/{{library_id}}.{{ref_name}}.{{align_method}}_emseq_methyldackel_dedup.log",
    params:
...
rule make_single_methylkit_amp_obj:
    ...
    input:
        f"{config['emseq-dir']}/meth/{{library_id}}.{{ref_name}}.{{align_method}}_methyldackel_CpG.methylKit",
    log:
        f"{config['emseq-dir']}/logs/{{library_id}}.{{ref_name}}.{{align_method}}_single_methylkit_amp.log",
    output:
...
rule emseq_biscuit_index:
    ...
    log:
        f"{config['emseq-dir']}/logs/{{name}}_biscuit_index.log"
    shell:
        """
...
https://pubmed.ncbi.nlm.nih.gov/24191000/
https://deeptools.readthedocs.io/en/stable/content/tools/plotProfile.html
https://pmc.ncbi.nlm.nih.gov/articles/PMC7849414/
