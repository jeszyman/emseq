...
jobs:
  run:
    ...
    steps:
      ...
      - name: Clone external analysis repos
        run: |
          set -euo pipefail
          chmod +x tools/setup_repos.sh
          SKIP_BUILD=1 REPOS_DIR="$HOME/repos" ./tools/setup_repos.sh
      - name: Build mHapTools from source
        run: |
          set -euo pipefail
          ...
          cp build/mhaptools .
      - name: Build wgbs_tools in deconv conda env
        run: |
          set -euo pipefail
