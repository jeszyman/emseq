...
- Add =Mk= read-level summary statistic: per-sample count (or fraction) of reads containing ≥k methylated CpGs. Captures read-level methylation density without collapsing to region mean — behaves like methylation haplotype load when k matches a region's CpG count. Related prior art: Pasha et al. 2026 BCMA targeted panel, Guardant =MR= metric, Guo et al. 2017 methylation haplotype load (MHL). Candidate pipeline use: per-DMR per-sample feature for classifier input or QC; complements the tiled-mean methylation currently produced by =make_methylkit_diff_db_tiled=.
#+begin_src snakemake
