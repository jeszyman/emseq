...
jobs:
  run:
    ...
    steps:
      ...
      # Build the same env you use locally (adjust path if needed)
      - name: Setup Conda (Mambaforge) with repo env
        uses: conda-incubator/setup-miniconda@v2
        with:
          miniforge-variant: Miniforge3
          auto-activate-base: false
          use-mamba: true
          activate-environment: emseq
          environment-file: config/emseq-conda-env.yaml

      # Optional: cache generated fixtures to speed reruns
      ...
      # NOTE: shell: bash -l logs into the conda-enabled shell so env is active
      - name: Execute script
        shell: bash -l {0}
