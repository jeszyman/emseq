...
#+end_src
*** Mosdepth
#+begin_src yaml :tangle ./config/mosdepth-conda-env.yaml :comments no
name: mosdepth
channels:
  - conda-forge
  - bioconda
...
- *Shell strict mode*: =samtools view -@ N= emits SIGPIPE (exit 141) under =set -euo pipefail=; wrapper must add =shell.prefix("set -e; ")= — consider documenting or adding to canonical =test.smk=
