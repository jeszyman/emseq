...
- Alignment and duplicate marking.
  Reads were aligned to reference genomes using BWA-meth with default parameters. Resulting BAMs were coordinate-sorted and duplicates were marked using dupsifter (https://github.com/brentp/dupsifter), which performs read name-aware duplicate detection optimized for bisulfite-style alignments. Only properly paired reads (SAM flag 0x2) were retained prior to duplicate marking. Duplicates were marked (SAM flag 0x400) but not removed at this stage, preserving the full read set for M-bias assessment.

- BAM filtering.
  Marked BAMs were filtered to retain only properly paired reads (SAM flag 0x2) with mapping quality ≥ 30, excluding secondary alignments, supplementary alignments, failed QC reads, and marked duplicates (SAM flag -F 3840). Reads were further intersected with autosomal keep regions and filtered against a blacklist BED.
