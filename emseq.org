...
- Development since last tag
  - [2026-03-25 Wed] AERO pipeline validated on GCP VM. Fixes for production run:
    - Sync org tangle blocks to use =rule_threads()= / =rule_concurrency()= helpers (was hardcoded).
    ...
    - Added =rule_threads= / =rule_concurrency= resource helpers to =aero-analysis.smk= wrapper.
    - Haplotype conda env: added =mhaptk=, =pandas=, =matplotlib=, =scipy=, =seaborn=, =tqdm= as pip deps.
    - =mHapTools= binary must be built from source on Linux (repo ships macOS Mach-O); requires =cmake= and bundled htslib built with =./configure && make=, then =sed 's/libhts.dylib/libhts.so/' CMakeLists.txt=.
    - CpG reference (=hg38_CpG.gz=) generated from hg38 FASTA via awk + bgzip/tabix (29.4M sites); upstream URL (bioinformatics.sibcb.ac.cn) returns 403.
    - MHB bed: use Guo 2017 liftover (=guo2017_mhb_hg38_sorted.bed=, 147K blocks), not de novo discovery (requires 30x+ coverage).
    - =annotatr= R package fails on GCP VMs due to conda SSL cert mismatch; workaround: serve =cpgIslandExt.txt.gz= locally or set =CURL_CA_BUNDLE=/etc/ssl/certs/ca-certificates.crt=.
  - [2026-03-24 Mon] Config-driven resource limits (threads/concurrency per rule via aero.yaml). Fixed mosdepth quantize string bug (single → global comma replacement). Added aero.smk project wrapper.
...
# -----------------------------
haplotype:
  cpg-ref: tests/full/ref/hg38_CpG_chr22.gz
  mhb-bed: tests/full/ref/test_mhb_chr22.bed
...
deconv:
  ...
  atlas: tests/full/ref/Atlas.U25.l4.hg38_test.tsv
