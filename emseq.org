...
jobs:
  run:
    ...
    steps:
      ...
      # Fail fast if the env file isn't where we expect
      - name: Verify env file exists
        run: |
          set -euo pipefail
          test -f config/emseq-conda-env.yaml || { echo "Missing: config/emseq-conda-env.yaml"; exit 1; }
          sed -n '1,30p' config/emseq-conda-env.yaml

      # Build env from your YAML (uses name: in the file; no activate-environment)
      - name: Setup Conda (Miniforge) from repo env
        ...
        with:
          ...
          activate-environment: emseq  # Add this line!
      ...
      # Use login shell so the Conda env is active (puts conda on PATH)
      - name: Execute script
        shell: bash -l {0}
